...
namespace HT.Framework
{
    ...
    /// 微信小游戏UI管理助手
    /// </summary>
    public class MiniGameUIHelper : IUIHelper
    {
        //UI管理器
        ...
        /// 所有World类型的UI
        /// </summary>
        public Dictionary<string, UIWorldDomain> WorldUIs { get; private set; } =
            new Dictionary<string, UIWorldDomain>();
        ...
        {
            set { _UIEntity.SetActive(!value); }
            get { return !_UIEntity.activeSelf; }
        }
        ...
        {
            set { _maskPanel.SetActive(value); }
            get { return _maskPanel.activeSelf; }
        }
        ...
        public void OnInit()
        {
            ...
            {
                return (type.IsSubclassOf(typeof(UILogicResident)) ||
                        type.IsSubclassOf(typeof(UILogicTemporary))) && !type.IsAbstract;
            });
            for (int i = 0; i < types.Count; i++)
            {
                ...
                {
                    throw new HTFrameworkException(HTFrameworkModule.UI,
                        "创建UI逻辑对象失败：UI逻辑类 " + types[i].Name + " 丢失 UIAttribute 标记！");
                }
            }
        }
        ...
        {
            UIAddressableAttribute attribute = type.GetCustomAttribute<UIAddressableAttribute>();
            if (attribute != null)
            {
                switch (attribute.EntityType)
                {
                    case UIType.Overlay:
                        ...
                        {
                            throw new HTFrameworkException(HTFrameworkModule.UI,
                                string.Format("预加载UI失败：UI对象 {0} 并未存在！", type.Name));
                        }
                    case UIType.Camera:
                    ...
                    case UIType.World:
                        if (WorldUIs.ContainsKey(attribute.WorldUIDomainName))
                        {
                            return WorldUIs[attribute.WorldUIDomainName].PreloadingResidentUI(type,
                                _defineUIAndEntitys.ContainsKey(type.FullName)
                                    ? _defineUIAndEntitys[type.FullName]
                                    : null);
                        }
                        else
                        {
                            throw new HTFrameworkException(HTFrameworkModule.UI,
                                string.Format("预加载UI失败：UI对象 {0} 的域 {1} 并未存在！", type.Name, attribute.WorldUIDomainName));
                        }
                }
            }
            ...
            if (attribute != null)
            {
                switch (attribute.EntityType)
                {
                    ...
                    case UIType.World:
                        if (WorldUIs.ContainsKey(attribute.WorldUIDomainName))
                        {
                            return WorldUIs[attribute.WorldUIDomainName].PreloadingTemporaryUI(type,
                                _defineUIAndEntitys.ContainsKey(type.FullName)
                                    ? _defineUIAndEntitys[type.FullName]
                                    : null);
                        }
            ...
            if (attribute != null)
            {
                switch (attribute.EntityType)
                {
                    case UIType.Overlay:
                        if (OverlayUIs.ContainsKey(type))
                        {
                            return CreateOpenUIEntity(attribute, type.FullName, OverlayUIs[type], _overlayResidentPanel,
                                args);
                        }
                        else
                        {
                            throw new HTFrameworkException(HTFrameworkModule.UI,
                                string.Format("打开UI失败：UI对象 {0} 并未存在！", type.Name));
                        }
                    case UIType.Camera:
                        if (CameraUIs.ContainsKey(type))
                        {
                            return CreateOpenUIEntity(attribute, type.FullName, CameraUIs[type], _cameraResidentPanel,
                                args);
                        }
                        else
                        {
                    ...
                    case UIType.World:
                        if (WorldUIs.ContainsKey(attribute.WorldUIDomainName))
                        {
                            return WorldUIs[attribute.WorldUIDomainName].OpenResidentUI(type,
                                _defineUIAndEntitys.ContainsKey(type.FullName)
                                    ? _defineUIAndEntitys[type.FullName]
                                    : null, args);
                        }
                        ...
                        {
                            throw new HTFrameworkException(HTFrameworkModule.UI,
                                string.Format("打开UI失败：UI对象 {0} 的域 {1} 并未存在！", type.Name, attribute.WorldUIDomainName));
                        }
                }
            }
                ...
                switch (attribute.EntityType)
                {
                    case UIType.Overlay:
                        if (OverlayUIs.ContainsKey(type))
                        {
                            ...
                            return CreateOpenUIEntity(attribute, type.FullName, OverlayUIs[type],
                                _overlayTemporaryPanel, args);
                        }
                        else
                        {
                            throw new HTFrameworkException(HTFrameworkModule.UI,
                    ...
                    case UIType.World:
                        if (WorldUIs.ContainsKey(attribute.WorldUIDomainName))
                        {
                            return WorldUIs[attribute.WorldUIDomainName].OpenTemporaryUI(type,
                                _defineUIAndEntitys.ContainsKey(type.FullName)
                                    ? _defineUIAndEntitys[type.FullName]
                                    : null, args);
                        }
        ...
        /// <returns>加载协程</returns>
        private Coroutine CreateUIEntity(UIAddressableAttribute uIResource, string uITypeName, UILogicBase uILogic,
            Transform uIParent)
        {
            ...
            {
                return Main.m_Addressable.LoadPrefab(new PrefabInfo(uIResource), uIParent, null, (obj) =>
                {
        ...
        /// <returns>加载协程</returns>
        private Coroutine CreateOpenUIEntity(UIAddressableAttribute uIResource, string uITypeName, UILogicBase uILogic,
            Transform uIParent, params object[] args)
        {
            if (uILogic.IsOpened)
                return null;
